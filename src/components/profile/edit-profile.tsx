...
const userSchema = z.object({
  ...
  username: z
    ...
    .refine(
      (value) =>
        !["home", "explore", "notifications", "terms", "privacy"].includes(
          value.toLowerCase(),
        ),
      {
...
export default function EditProfile() {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (user) {
      ...
      if (bannerFile) {
        await uploadBanner(bannerFile, user.id);
      }
    ...
    setOpen(false);
  }, [formData, user, avatarFile, bannerFile]);
        ...
        <div className="relative h-40">
          <Image
            src={
              bannerPreview ||
              user.banner ||
              "/images/default-profile-banner.webp"
            }
            alt="Cover image"
            layout="fill"
            objectFit="cover"
          ...
          <Label
            htmlFor="banner"
            className="absolute top-2 right-2 cursor-pointer bg-black/50 text-white p-2 rounded-full z-10"
          >
            <Input
              id="banner"
              type="file"
