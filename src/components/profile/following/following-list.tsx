...
export function FollowingList({ username, searchQuery, id }: FollowersListProps) {
  ...
  // Configuración para detectar cuando el usuario llega al final de la lista
  const { ref, inView } = useInView({
    threshold: 0.5,
  })

  // Usar SWR para obtener los datos y manejar la recarga automáticamente
  const { data, error, isLoading } = useSWR(`/user/${id}/following`, fetcher, {
    revalidateOnFocus: false, // Evitar revalidación al enfocar la ventana
    dedupingInterval: 60000, // Deduplicar solicitudes durante 1 minuto
  })
  ...
  // Cargar más seguidores cuando se llega al final de la lista
  useEffect(() => {
    if (inView && hasMore && !isLoading && !loadingMore) {
      const loadMoreFollowers = async () => {
        ...
        try {
          // Aquí deberías hacer una llamada a la API con paginación
          // Por ejemplo: `/api/user/${id}/followers?page=${page + 1}`
          ...
          if (moreData && moreData.length > 0) {
            ...
            setPage((prev) => prev + 1)
            setHasMore(moreFollowers.length >= 10) // Si hay menos de 10, probablemente no hay más
          } else {
            setHasMore(false)
          }
        } catch (err) {
